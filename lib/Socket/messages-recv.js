...
const makeMessagesRecvSocket = (config) => {
    ...
    const ingestRelayData = async (callId, relayNode) => {
    ...
    //     audio(16000) → [video] → encopt(keygen=2) → capability(...bb 07)
    //   </preaccept></call>
    // ---------------------------------------------------------------------
    ...
    const acceptCall = async (callId, callFrom, opts = {}) => {
        ...
        // The real callee <accept> (wacrg SIG-05, meowcaller sendAccept): single
        // audio rate, a <net medium="2"/> child, encopt, and the Android A/B
        // metadata attr. NO rekey <enc>, NO capability — "capability+both-rates
        // fails" (the peer drops the call). The offer's callKey stays THE key.
        const sendAccept = async () => {
            const content = buildCallMediaContent(useVideo)
    ...
    const joinGroupCall = async (groupJid, audioInput, opts = {}) => {
        ...
        return connectCall(callId, callFrom, audioInput, { ...opts, group: true, groupJid: group, relayTimeoutMs: opts.relayTimeoutMs || 60000 })
    }
    ...
    const connectCall = async (callId, callFrom, audioInput, opts = {}) => {
        ...
        // wait for: callKey + relay (1:1), plus roster + epoch (group)
        for (;;) {
            cached = await callOfferCache.get(callId)
            if (!cached) {
                throw new Boom('offer cache entry disappeared mid-connect')
            }
            const callKey = cached.callKey || (cached.offerNode ? await decryptCallKey(callId, callFrom) : null)
            const relayReady = !!(cached.relay?.key?.length && cached.relay?.endpoints?.length)
            const groupReady = !isGroup || (
                (groupEpochs.get(callId) || cached.groupEpoch) &&
                ((groupRosters.get(callId)?.pids?.length) || cached.groupPids?.length)
            )
        ...
        cached = await callOfferCache.get(callId)
        const callKey = cached.callKey || await decryptCallKey(callId, callFrom)
        if (!callKey) {
    ...
    const handleCall = async (node) => {
        try {
            ...
            if (relayNode && callId) {
                const merged = await ingestRelayData(callId, relayNode)
            ...
            // -----------------------------------------------------------------
            // relaylatency: the caller probes relay RTT; echo each <te> back
            // (latency re-encoded, same relay_name + address bytes).
            // -----------------------------------------------------------------
            if (actionTag === 'relaylatency' && callId) {
                const tes = getAllBinaryNodeChildren(infoChild).filter((c) => c?.tag === 'te')
                for (const te of tes) {
            ...
            if (actionTag === 'group_update' && callId) {
                try {
                    ...
                    if (roster) {
                        ...
                        logger.info({ callId, participants: roster.participants.length, pids: roster.pids.length }, '[MOD] group roster updated')
                    }
                } catch (e) {
            ...
            if (status === 'offer') {
                ...
                if (call.isGroup) {
                    ...
                    if (roster) {
                        ...
                        mergedCall.groupPids = roster.pids
                        await callOfferCache.set(call.id, { ...mergedCall, groupPids: roster.pids })
                    }
