...
const makeMessagesRecvSocket = (config) => {
    ...
    // ---------------------------------------------------------------------
    const findEncNodes = (node, out = []) => {
        if (!node || typeof node !== 'object') return out
        if (node.tag === 'enc' && (node.attrs?.type === 'pkmsg' || node.attrs?.type === 'msg')) {
            out.push(node)
        }
        if (Array.isArray(node.content)) {
            for (const c of node.content) findEncNodes(c, out)
        }
        return out
    }

    const decryptCallKey = async (offerNode, callFrom) => {
        const encNodes = findEncNodes(offerNode)
        if (!encNodes.length) {
            logger.warn('[MOD] decryptCallKey: no enc node found in offer')
            return null
        }
        for (const enc of encNodes) {
            try {
                ...
                try {
                    ...
                    // some keys arrive without proto padding wrapper
                    msg = proto.Message.decode(plaintext.subarray ? plaintext.subarray(1, plaintext.length - (plaintext[plaintext.length - 1] || 0)) : plaintext)
                }
                const callKey = msg?.call?.callKey
                if (callKey && callKey.length) {
                    const hex = Buffer.from(callKey).toString('hex')
                    logger.info({ callFrom, callKeyHex: hex, len: callKey.length }, '[MOD] DECRYPTED media callKey')
                    return Buffer.from(callKey)
                }
            } catch (e) {
                logger.debug({ err: e?.message }, '[MOD] decryptCallKey: enc attempt failed, trying next')
            }
        }
        logger.warn('[MOD] decryptCallKey: could not extract callKey from any enc node')
        return null
    }
    ...
    const acceptCall = async (callId, callFrom) => {
        const encKey = randomBytes(32)
        const devices = (await getUSyncDevices([callFrom], true, false))
        ...
        const acceptContent = []
        acceptContent.push({ tag: 'audio', attrs: { enc: 'opus', rate: '16000' }, content: undefined })
        acceptContent.push({ tag: 'audio', attrs: { enc: 'opus', rate: '8000' }, content: undefined })
        acceptContent.push({ tag: 'net', attrs: { medium: '3' }, content: undefined })
        acceptContent.push({ tag: 'capability', attrs: { ver: '1' }, content: new Uint8Array([1, 4, 255, 131, 207, 4]) })
        acceptContent.push({ tag: 'encopt', attrs: { keygen: '2' }, content: undefined })
        acceptContent.push({ tag: 'destination', attrs: {}, content: destinations })
        ...
        await query(stanza)
        return { id: callId, callKey: Buffer.from(encKey).toString('hex') }
    }
    ...
    const connectCall = async (callId, callFrom, audioInput, opts = {}) => {
        ...
        const session = new WACallMediaSession({ callId, callKey, candidates, logger })
        callMediaSessions.set(callId, session)

        const pair = await session.connect(opts.iceTimeoutMs || 5000)
    ...
    const handleCall = async (node) => {
        ...
        if (status === 'offer') {
            ...
            // [MOD] auto-decrypt the media callKey from the offer. This is the
            // real, working RE step — it surfaces the clear 32-byte key that
            // any media-transport attempt would have to build on.
            try {
                const callKey = await decryptCallKey(infoChild, call.from)
                if (callKey) {
        ...
        if (status === 'reject' || status === 'accept' || status === 'timeout' || status === 'terminate') {
            ...
            if (status === 'terminate' || status === 'timeout' || status === 'reject') {
    ...
    return {
        ...
        offerCall, 
        rejectCall,
        acceptCall, // [MOD] signaling-level accept
